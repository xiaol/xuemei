...
from userena.serializers import UserenaSerializer, PaginatedUserSerializer, UserDetailSerializer
from profiles.serializers import ProfileSerializer, PaginatedProfileSerializer
...
class ProfileListView(ListAPIView):
    ...
    def get_queryset(self):
    	queryset = get_profile_model().objects.get_visible_profiles(self.request.user)
	return queryset
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if (userena_settings.USERENA_SIGNIN_AFTER_SIGNUP and
                ...
                login(request, user)
            serializer = UserenaSerializer(user)
...
def signin(request, auth_form=AuthenticationForm,
    ...
    **Context**
    ...
    if request.method == 'POST':
		...
		serializer = UserenaSerializer(user)
                return Response(serializer.data)#HttpResponseRedirect(redirect_to)
            else:
 		return Response({'msg':'user is disabled'})	
	else:
	    return Response(form.errors)
...
def profile_list(request, page=1, template_name='userena/profile_list.html',
    ...
    It's possible to disable this by
    return Response(serializer.data)
    return Response(serializer.data) changing ``USERENA_DISABLE_PROFILE_LIST``
...
def user_following(ListAPIView):
    """ get following list """
    ...
    def get_queryset(self):
        queryset = Follow.objects.get_following(request.user,User)
	return queryset

def user_follower(ListAPIView):
   """ get follower list """
   serializer_class = UserDetailSerializer
   paginate_by=12
   paginate_by_param = 'page_size'
   ...
   def get_queryset(self):
       queryset = Follow.objects.get_follows(request.user)
       return queryset
